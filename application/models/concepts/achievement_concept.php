...
class Achievement_concept extends OSA_Concept
{
	...
	public function set_id($achievement_id = 0)
	{
	...
	public function get_comments($top_id = NULL, $offset = 0, $row_count = 5)
	{
		...
		$ac_section_id = $CI->flags->get_section_id('achievement_comment');
		$ac_lock_section_id = $CI->flags->get_section_id('achievement_comment_lock');
		...
		$this->db
			...
			->select('(SELECT COUNT(*) FROM flags AS f WHERE f.section_id = ' . $ac_section_id . ' AND f.table_id = ac.id AND f.solved_by IS NULL) AS flag_count', FALSE)
			->select('(SELECT COUNT(*) FROM flags AS f WHERE f.section_id = ' . $ac_lock_section_id . ' AND f.table_id = ac.id AND f.solved_by IS NULL) AS flag_locked', FALSE)
