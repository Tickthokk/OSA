...
class User_concept extends OSA_Concept
{
	public 
		$is_logged = FALSE,
		$acl = NULL;
	...
	public function change_acl($level)
	{
		// No ACL?  Try to delete it.
		if ($level == 0)
			return $this->delete_acl();

		// ACL exists?  Update it.
		if ($this->get_acl())
			return $this->update_acl($level);

		return $this->create_acl($level);
	}

	public function update_acl($level)
	{
		$this->db
			->set('level', (int) $level)
			->where('uid', $this->id)
	...
	public function create_acl($level)
	{
		$this->db->insert('user_acl', array(
			'uid' => $this->id,
			'level' => (int) $level,
			'key' => md5($this->CI->config->item('encryption_key'))
		));
	}

	public function delete_acl()
	{
		$this->db->delete('user_acl', array(
			'uid' => $this->id
		));
	}
	 ...
	 * Ban Status
	 */
	public function change_ban($banned, $ban_reason)
	{
		$this->enable_set(array('banned', 'ban_reason'));
		$this->banned = (int) $banned;
		$this->ban_reason = $banned ? $ban_reason : NULL;
	}

	/**
	 * Active Status
	 */
	public function change_active($active)
	{
		$this->enable_set('activated');
		$this->activated = (int) $active;
	}
	...
	/**
	 * Password Test
	 *  Test the current users password against the given password
	 * @param string $password
	 * @return boolean >> TRUE if passwords match
	 */
	public function password_test($password)
	{
		return $this->_hash($password) == $this->password;
	}
